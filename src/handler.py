...
def get_dir(solution: Solution, path: Path) -> Path:
    ...
    """
    return path / solution.language / solution.kyu / solution.title.replace(r'/\<>:"|?*', '')
...
def write_solutions_to_files(solutions: List[Solution], path: Path, folder_name: str = 'CodeWars') -> None:
    """
    ...
    for solution in solutions:
        if solution.kyu in stat: stat[solution.kyu] += 1
        else: stat[solution.kyu] = 1

        path_to_solution = get_dir(solution, path)
        path_to_solution.mkdir(parents=True, exist_ok=True)

        create_file(path_to_solution / f'main{EXTENSIONS[solution.language]}',
            solution.solution)

        create_file(path_to_solution / 'README.md',
            f'Link: {solution.link}')

    kuy_stat = '\n'.join([f":star: {x}: {stat[x]} :star:" for x in sorted(stat)])
    create_file(path / 'README.md', f'Total: {sum([stat[x] for x in stat])}\n'
        f'Detail statistic:\n{kuy_stat}')
