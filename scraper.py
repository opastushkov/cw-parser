...
def authorization(s: Session, token: str) -> None:
    """
...
def get_page_with_solutions(s: Session, page_number: int) -> BeautifulSoup:
    ...
    page_with_solutions = BeautifulSoup(
        s.get(
            url=BASE_URL + 'users/' + USERNAME + '/completed_solutions?page=' + str(page_number),
            headers=HEADERS
        ).text,
    )
...
def get_solutions(page: BeautifulSoup) -> List[BeautifulSoup]:
    ...
    """
    solutions_list = page.find_all('div', {'class': 'list-item solutions'})
...
def get_all_solutions() -> List[Solution]:
    ...
    with Session() as session:
        ...
        authorization(session, authenticity_token)
